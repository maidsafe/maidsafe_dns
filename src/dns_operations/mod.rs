...
impl DnsOperations {
    ...
    /// Get the messaging encryption keys that the user has associated
    /// with one's particular Dns-name.
    ...
    /// Get the home directory (eg., homepage containing HOME.html, INDEX.html)
    ...
    fn add_remove_service_impl(&self,
        ...
        if !is_add_service && !dns_record.services.contains_key(&service.0) {
            ...
            if is_add_service {
                ...
                let _ = dns_record.services.insert(service.0,
                                                   try!(service.1.ok_or(DnsError::from("Progra\
                                                                                        mming \
                                                                                        Error \
                                                                                        - Inve\
                                                                                        stigat\
...
mod test {
    ...
    fn register_and_delete_dns() {
        ...
        unwrap_result!(unwrap_result!(client.lock())
                           .delete(Data::StructuredData(struct_data), None));
    ...
    fn manipulate_services() {
        ...
        let home_dir_key = unwrap_result!(dns_operations_unregistered
                .get_service_home_directory_key(&dns_name, &services[1].0, None));
        ...
        // match dns_operations.get_service_home_directory_key(&dns_name,
        //                                                     &removed_service.0, None) {
        //     Ok(_) => panic!("Should have been an error"),
        //     Err(DnsError::ServiceNotFound) => (),
        //     Err(error) => panic!("{:?}", error),
        // }
